...
task presto_clontech{

    input {
        File R1_FILE #ERR346600_1.fastq
        File R2_FILE #ERR346600_2.fastq
        File CREGION #/usr/local/share/protocols/Universal/Human_IG_CRegion_RC.fasta
        ...
        # Docker command
        docker run -v $DATA_DIR:/data:z immcantation/suite:4.1.0 \
        presto-clontech -1 $READS_R1 -2 $READS_R2 -j $CREGION -r $VREF \
        -n $SAMPLE_NAME -o $OUT_DIR -p $NPROC

    command {
        run -v $DATA_DIR:/data:z immcantation/suite:4.1.0 \
            presto-clontech -1 ~{R1_FILE} -2 ~{R2_FILE} -j $CREGION -r $VREF \
            -n $SAMPLE_NAME -o $OUT_DIR -p $NPROC
    }
